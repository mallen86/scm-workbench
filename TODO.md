...
- [x] Add a notification message in the sidebar for jobs that are currently running that transitions into a done message the disappears after 5 seconds or after the X is clicked in the top right of the notification
- [x] If the clear card images button is clicked on the fetch card art page, remove the done message box so we don't direct users to create a pdf with no images
- [x] We need to add some indication that images are missing during/after an image fetch. The MPCFill plugin can fail to fetch images in the first stage and then throw a warning in the second stage. Stage 1 can throw 404 errors but it's not always clear if it's a terminal failure or an initial failure that gets retried successfully. The second stage is much clearer. Let's add an orange circle warning icon to the left of the progress bar when stage 1 shows multiple 404 errors and a red triangle warning icon when stage 2 shows the warning about no image data found.
    ```
...
- [x] Add a toggle for skipping the bottom left image. The id of the bottom left image changes depending on if borderless is seleted and what paper type is selected. This will use the `--skip` option. Letter/non-borderless 4, letter/borderless 6, a4/non-borderless 4, a4/borderless 6, a3/non-borderless 12, a3/borderless 12, tabloid/non-borderless 12, tabloid/borderless 12, arch_b/non-borderless 12, arch_b/borderless 14, legal/non-borderless 5, legal/borderless 5. Note legal is added when using scm off main but add the handler for it no matter the scm version.
- [x] Always use `--extend_corners 3.5mm` in simple mode and make 3.5mm the default in advanced mode. Users can still change the setting in advanced mode but it should just be 3.5mm out of the box.
