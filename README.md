...
**Interface: Simple or Advanced** (the switch in the top bar, next to the Console button — it applies to every page). Advanced is the full layout above. Simple is about not being overwhelmed: the navigation collapses to just **Fetch card art**, **Create PDF** and Settings, and the Create PDF page shows only the basic settings — card & paper size, registration marks, borderless, front-only and the saved-offset switch; directory fields, resolution/quality and the power sections stay one click away. The Fetch pages are already as simple as they get, so they're untouched in either mode; hidden options simply keep their defaults, so the command that runs is identical in both modes.
