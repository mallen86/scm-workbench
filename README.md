...
When setup finishes, the welcome screen offers an optional six-step tutorial for fetching card art, choosing card and paper sizes, adding a card back, and creating a PDF. Stop it at any step or replay it later from **Settings → Guided tutorial**. Completing all six steps returns to **Fetch card art**.
...
* **Create PDF includes a low quality first-page preview.** Workbench normalizes a bounded representative selection of fronts, fills every available position in the verified first-page layout, renders only private low resolution copies with the unchanged upstream `create_pdf.py`, and displays the result as a bounded JPEG. Preview work is cancellable and disposable. It does not modify card folders, normal output, jobs, history, logs, or export permissions.
