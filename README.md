...
* **Repository metadata is asynchronous in the packaged app.** Ref lists, source selection, and update checks use native IPC starts followed by bounded polls. Remote GitHub work runs in the worker's background registry instead of blocking the serialized transport; downloading or updating a managed copy remains a normal `jobs.start` job. A browser uses the existing HTTP routes, while a packaged native failure is shown rather than silently retried over HTTP.
...
Running from source, packaging the app, and cutting releases are documented in [CONTRIBUTING.md](CONTRIBUTING.md). The current transitional native boundary, including bounded `settings.set`, `offset.set`, `offset.delete`, `file.open`, `file.reveal`, `url.open`, and asynchronous repository metadata IPC, is documented in [docs/native-ipc.md](docs/native-ipc.md). Browser fallback, raw bytes, save/delete, and updates remain HTTP; the packaged smoke guard rejects WebView repository metadata POSTs after the WebKit marker, while HTTP itself has not been removed.
