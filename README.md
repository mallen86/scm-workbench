...
**Interface: Simple or Advanced** (the switch in the top bar, next to the Console button — it applies to every page). Advanced is the full layout above. Simple is about not being overwhelmed: the navigation collapses to just **Fetch card art**, **Create PDF** and Settings, and the Create PDF form becomes a single flat section — no group headers and no collapsible power sections, every setting (the folder fields, card & paper size, registration marks, quality, fit & edge finishing, advanced) just runs on in order, and its card title drops out too, leaving one plain, unbroken form. The Fetch pages are already as simple as they get, so they're untouched in either mode.
