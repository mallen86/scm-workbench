...
Running from source, packaging the app, and cutting releases are documented in [CONTRIBUTING.md](CONTRIBUTING.md). Packaged Tauri embeds only the bounded `ui/` tree and serves `index.html` plus its root-relative assets from the app origin; its WebView never navigates to the worker's loopback HTTP listener. The worker HTTP listener remains transitional compatibility support for standalone browser flows. The current transitional native boundary, including bounded `settings.set`, offsets, OS actions, secure image deletion, decklist import, artifact export, asynchronous repository metadata, and app update/release-notes IPC, is documented in [docs/native-ipc.md](docs/native-ipc.md). Browser fallback and raw bytes remain standalone-browser HTTP compatibility; packaged `/api/file` is unavailable because the UI has no raw-file caller. Standalone browsers retain POST `/api/fs` while packaged image deletion uses native IPC only. The packaged smoke guard uses native IPC markers as rendered-UI evidence and rejects any WebView HTTP request; the transitional HTTP listener itself remains for standalone browsers.
