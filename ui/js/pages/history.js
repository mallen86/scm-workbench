/* pages/history: the dedicated Job history page.
   Available in both interface modes: it lists every job the workbench has
   run (live and persisted) and, when a job has a page, opens that page with
   the exact settings the job ran with. The list itself lives in
   job-history.js so the jobs poll can repaint it in place; go() asks
   refreshJobs(true) for a fresh paint on arrival. */
