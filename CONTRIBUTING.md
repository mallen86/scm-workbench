...
* **Native boundary (current migration wave).** The packaged Tauri window supervises one Python worker. That worker serves both the existing HTTP compatibility server and bounded JSON-lines RPC over stdin/stdout. Bootstrap reads, preview, packaged-Tauri job list/start/log/kill/poll, and the read-only `template.resolve`/`file.list` metadata slice use native IPC; browser HTTP/SSE fallback, binary reads, file open/reveal actions, settings writes, repo actions, filesystem operations, update-start, and static/worker-origin navigation remain HTTP. See [docs/native-ipc.md](docs/native-ipc.md) for the exact schema, bounds, ACL, rollout rule, and verification commands.
