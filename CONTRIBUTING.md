...
* **Live logs** use native aggregate polling for packaged Tauri windows and a
  ...
  packaged).
* The Workbench never imports code from the base repos — it reads their JSON and invokes their scripts in child processes. Capability discovery keeps forms compatible with older checkouts by exposing only CLI flags their own help output advertises; unsupported required flags disable the workflow rather than launching a command known to fail. `silhouette-card-maker` and `scm-extras` remain authoritative for fetching, PDF/DXF generation, and layouts; Workbench only wraps and orchestrates them.
