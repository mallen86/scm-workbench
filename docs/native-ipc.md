...
absolute UTF-8 path to the embedded Settings page or a manifest-driven job form;
cancellation returns JSON `null`. Paths are capped at 4096 UTF-8 bytes and
control characters are rejected before the UI can place the value in a field.
Settings persistence still goes through the bounded `settings.set` operation;
job-form paths remain ordinary preview/start arguments. Standalone browsers
retain editable path inputs and reset controls but do not display a browse
control because browser file APIs cannot provide a usable absolute directory
path. A selected native bridge never retries through HTTP after a failure.
...
| Representative Create PDF preview | Bounded Tauri start/poll/cancel JSON-lines in packaged windows; POST `/api/pdf-preview` in standalone browsers | Reads at most 16 private copied source fronts, repeats only normalized private copies to fill the verified first-page layout, uses fixed low-resolution overrides, a 15-second deadline, process-tree cancellation, bounded Pillow decoding/JPEG output, and disposable private output. It invokes unchanged upstream `create_pdf.py` and never publishes normal job or artifact state. |
